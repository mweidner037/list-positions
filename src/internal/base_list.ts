...
import { DeletedSegment, Segment, SegmentList } from "./segment";

/**
 * List data associated to a BunchNode.
 */
class NodeData<S extends Segment<S>> extends SegmentList<S> {
  constructor(
    /**
     ...
     *
     * For nodes without NodeData, call BaseList.parentValuesBefore instead
     * of defaulting to 0.
     */
    public parentValuesBefore: number
  ) {
    super();
  }
}

export class BaseList<S extends Segment<S>> {
  /**
  ...
  set(startPos: Position, segment: S): SegmentList<S> {
    ...
    const replaced = data.overwrite(startPos.innerIndex, segment);
   ...
   * @returns Replaced values
   */
  delete(startPos: Position, count: number): SegmentList<S> {
    // Validate startPos even if count = 0.
    const node = this.order.getNodeFor(startPos);
    if (count === 0) return new SegmentList();
    if (node === this.order.rootNode && startPos.innerIndex + count - 1 > 1) {
    ...
    const replaced = data.overwrite(
      startPos.innerIndex,
      new DeletedSegment(count)
    );
  ...
  *dependencies(): IterableIterator<BunchMeta> {
  ...
  /**
   * Returns a saved state for this BaseList.
   *
   * The saved state describes our current (Position -> value) map in JSON-serializable form.
   * You can load this state on another BaseList by calling `load(savedState)`,
  ...
  /**
   * Returns a saved state for the Outline corresponding to this BaseList.
   *
  ...
  /**
   * Loads a saved state returned by another BaseList's `save()` method.
   *
   * Loading sets our (Position -> value) map to match the saved BaseList's, *overwriting*
   * our current state.
   *
